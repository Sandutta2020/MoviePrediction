...
def getting_recommended_movie(moviename):
    ...
    df_kwds=df_kwds.sort_values(by=moviename,ascending=False)[[moviename]].head(5)
    print(df_kwds)
    df_new =df_kwds.copy()
    print(type(df_new))
    df_new['SearchType']="Keywords and Genre Based"
    df_user_rev=pd.read_csv('data/user_review_similarity.gzip',compression='gzip',index_col='Title')
    df_user_rev=df_user_rev.sort_values(by=moviename,ascending=False)[[moviename]].head(5)
    df_user_rev['SearchType']="User Recommened"
    print(df_user_rev)
